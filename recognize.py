...
def getFeatures(img):
    ...
    kp, des = detector.detectAndCompute(Igray, None)
    return kp, des, img.shape[:2][::-1]

def detectFeatures(img, train_features):
    train_kps, train_desc, shape = train_features
    kps, desc, _ = getFeatures(img)
    ...
    try:
        ...
        src_pts = np.float32(
                [train_kps[m[0].queryIdx].pt for m in good]
                ).reshape(-1,1,2)
        dst_pts = np.float32(
                [kps[m[0].trainIdx].pt for m in good]
                ).reshape(-1,1,2)
...
img = cv2.imread('starbuko_logo/logo1.jpg')
